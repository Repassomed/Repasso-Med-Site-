...
AUDIT_SYSTEM_PROMPT = (
    ...
    "A auditoria é RELATIVA AO ESCOPO REAL DO DIFF. Não transforme uma PR "
    "estreita em saneamento geral de dívida editorial pré-existente: deficiência "
    "em conteúdo que o diff não toca não bloqueia MERGE-READY por si só. Ela só é "
    "bloqueante se a mudança a criar, piorar, depender dela, apagar evidência "
    "necessária, ou se o próprio objetivo da tarefa for corrigi-la. Em remoções/"
    "reorganizações, confirme que o que saiu não sustentava o raciocínio ou as "
    "questões; não exija reescrever bancos de questões intocados só para provar "
    "conformidade histórica. Registre julgamento apenas sobre impacto verificável "
    "desta mudança. "
