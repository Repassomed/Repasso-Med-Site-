...
distratores, cobertura.

8-A.1 Localização dupla obrigatória

Toda questão vinculada a um conteúdo específico deve existir em DOIS lugares:

1. no FINAL DO BLOCO correspondente ao assunto ensinado;
2. no BANCO GERAL de questões da matéria.

Não é "um ou outro". A regra é: BLOCO ESPECÍFICO + BANCO GERAL.

As duas cópias devem representar a MESMA questão, com o mesmo enunciado, as
mesmas alternativas, o mesmo gabarito, a mesma explicação e a mesma
proveniência. A cópia do bloco leva id (por exemplo id="q-orto438"); a cópia
do banco não leva id, para não duplicar identificadores no DOM.
...
8-A.3 Questões baseadas em prova
...
8-A.4 Extração de foto, PDF, documento ou memória parcial
...
- exatamente UMA melhor resposta;
- distratores plausíveis;
- dificuldade compatível com prova médica;
- pelo menos DUAS alternativas muito próximas entre si.
...
8-A.6 Distribuição das letras corretas
...
explicação continuam válidas. Ao aplicar a troca é obrigatório remapear
também as letras citadas dentro do bloco .answer (o "a)", o "b) y c)" etc.)
e aplicar a MESMA troca à cópia gêmea do banco geral.
...
Se a questão nova for duplicata conceitual ou exata de uma já existente,
comparar antes de inserir e decidir entre preservar a existente sem
duplicar, ou integrar a nova formulação apenas se ela trouxer valor real
(outro formato, outro recorte, outro eixo de confusão). Nunca apagar
silenciosamente. Registrar no relatório qual foi a decisão e por quê.

8-A.9 Contagens declaradas
...
8-A.10 Relatório obrigatório da rodada

Informar: questões recebidas, incorporadas, duplicadas, pendentes,
reconstruídas com alternativas e não incorporadas com o motivo; cobertura
(novas cobertas, antigas auditadas, parciais, lacunas encontradas e
corrigidas); banco (presentes nos blocos, presentes no banco geral,
divergências); gabaritos (A/B/C/D, maior sequência, padrão periódico antes e
depois, métrica); e as pendências que precisam de decisão humana.
