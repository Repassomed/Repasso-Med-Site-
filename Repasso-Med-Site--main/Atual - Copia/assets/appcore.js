...
var RepassoMed = (function(){
  ...
  function setupMenu(tabEl, nav){
    ...
    /* v3 · 31/08/2026 — «só acerta no segundo clique»
       -----------------------------------------------
       O styles.css põe `content-visibility:auto` com
       `contain-intrinsic-size: 1px 2400px` em TODA seção. Enquanto uma
       seção não é renderizada ela vale 2400px; quando o scroll chega
       perto, ela materializa e passa a valer o tamanho real — que nas
       matérias grandes é dez ou vinte vezes mais. Ou seja: o alvo se
       DESLOCA durante o próprio salto.

       A versão anterior fazia UMA correção aos 420ms. Nesse instante o
       scroll suave muitas vezes ainda estava a caminho e havia seções por
       materializar, então a correção era medida sobre um layout que ainda
       ia mudar — daí cair no bloco vizinho. No segundo clique tudo já
       estava renderizado e por isso acertava.

       Agora corrigimos em laço até a posição ESTABILIZAR (duas medidas
       seguidas no lugar), com teto de tempo. E se o usuário rolar por
       conta própria, o laço desiste na hora — nunca brigamos com ele. */
    var ajusteAtivo = null;
    function irPara(id){
      var alvo = document.getElementById(id);
      if (!alvo) return;

      if (ajusteAtivo) { ajusteAtivo.cancelar(); ajusteAtivo = null; }

      function desvio(){ return alvo.getBoundingClientRect().top - alturaFixa(); }

      // 1.º passo: suave, para não teletransportar a tela
      window.scrollTo({ top: Math.max(0, alvo.getBoundingClientRect().top + window.pageYOffset - alturaFixa()), behavior: 'smooth' });

      // 2.º: acertos sucessivos até estabilizar (máx. ~2,4s)
      var t0 = Date.now(), okSeguidos = 0, timer = null, vivo = true;
      function desistir(){
        if (!vivo) return;
        vivo = false;
        if (timer) clearTimeout(timer);
        window.removeEventListener('wheel', desistir, {passive:true});
        window.removeEventListener('touchstart', desistir, {passive:true});
        window.removeEventListener('keydown', desistir, true);
        ajusteAtivo = null;
      }
      ajusteAtivo = { cancelar: desistir };
      // qualquer rolagem do usuário encerra o acerto
      window.addEventListener('wheel', desistir, {passive:true});
      window.addEventListener('touchstart', desistir, {passive:true});
      window.addEventListener('keydown', desistir, true);

      (function acerta(){
        if (!vivo) return;
        var d = desvio();
        if (Math.abs(d) <= 2) { okSeguidos++; }
        else { okSeguidos = 0; window.scrollBy({ top: d, behavior: 'auto' }); }
        if (okSeguidos >= 2 || Date.now() - t0 > 2400) { desistir(); return; }
        timer = setTimeout(acerta, 90);
      })();
...
/* =====================================================================
   AMPLIAR IMAGEN (.rm-fs) — higiene global · 31/08/2026
   ---------------------------------------------------------------------
   Las materias prácticas amplían una foto con
   `onclick="this.classList.toggle('rm-fs')"` y CSS `position:fixed`.
   Eso funciona, pero faltaban tres cosas y el resultado parecía «el
   sitio dio error»:

   1) Al tocar una segunda foto quedaban DOS ampliadas, una encima de la
      otra. Al cerrar la de arriba seguía la de abajo tapando la pantalla.
   2) No cerraba con Escape ni tocando fuera de la imagen.
   3) styles.css pone `content-visibility:auto` en cada sección, que
      implica `contain:paint`: eso convierte la sección en contenedor de
      los `position:fixed` de adentro, así que la foto ampliada se
      anclaba a la SECCIÓN y no a la ventana — aparecía descolocada, a
      veces fuera de la pantalla.

   Se resuelve acá, en el motor, para que valga en TODAS las materias sin
   tocar ningún fragmento (§6: cero <script> dentro de la materia).
   ===================================================================== */
(function(){
  var SEC = 'section.container, section.container-wide';

  function sincronizar(){
    document.querySelectorAll('.rm-fs-host').forEach(function(s){ s.classList.remove('rm-fs-host'); });
    document.querySelectorAll('.rm-fs-back').forEach(function(d){ d.parentNode && d.parentNode.removeChild(d); });

    var abierta = document.querySelector('.rm-fs');
    document.documentElement.classList.toggle('rm-fs-abierto', !!abierta);
    if (!abierta) return;

    var s = abierta.closest ? abierta.closest(SEC) : null;
    if (s) s.classList.add('rm-fs-host');

    /* fondo oscuro: hermano de la foto, para compartir contexto de apilamiento */
    var fondo = document.createElement('div');
    fondo.className = 'rm-fs-back';
    fondo.setAttribute('aria-hidden','true');
    abierta.parentNode.insertBefore(fondo, abierta);
  }

  function cerrarTodas(menos){
    document.querySelectorAll('.rm-fs').forEach(function(o){ if (o !== menos) o.classList.remove('rm-fs'); });
  }

  /* Fase de captura: corre ANTES del onclick del marcado. */
  document.addEventListener('click', function(e){
    var t = e.target;
    var esAmpliada = t && t.classList && t.classList.contains('rm-fs');
    if (esAmpliada){ cerrarTodas(t); return; }   // el onclick la va a cerrar
    // se tocó otra cosa: si había alguna ampliada, se cierra
    cerrarTodas(null);
  }, true);

  /* Fase de burbujeo: el onclick del marcado ya corrió, ahora sincronizamos. */
  document.addEventListener('click', sincronizar, false);

  document.addEventListener('keydown', function(e){
    if (e.key === 'Escape' || e.key === 'Esc'){ cerrarTodas(null); sincronizar(); }
  });
})();
